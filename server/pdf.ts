...
export async function buildCasePack(record: CaseRecord) {
  ...
  function addPage(title: string, kicker: string) {
    ...
    page.drawText(record.reference, { x: 474, y: 801, font: bold, size: 8, color: colors.white })
  ...
  summary.drawText(record.claimantName, { x: 57, y: 583, font: bold, size: 11, color: colors.ink })
  summary.drawText('RESPONDENT', { x: 233, y: 604, font: bold, size: 6.5, color: colors.muted })
  summary.drawText(record.respondentName.slice(0, 38), { x: 233, y: 583, font: bold, size: 11, color: colors.ink })
  ...
  summary.drawText(record.courtStation, { x: 57, y: 556, font: body, size: 7.5, color: colors.muted })
  ...
  record.evidence.slice(0, 7).forEach((item, index) => {
    ...
    evidence.drawText(item.name.slice(0, 58), { x: 92, y: evidenceY - 19, font: bold, size: 8.5, color: colors.ink })
  ...
  record.timeline.slice(0, 6).forEach((event) => {
    ...
    evidence.drawText(event.title, { x: 158, y: evidenceY, font: bold, size: 8.5, color: colors.ink })
    evidenceY = drawParagraph(evidence, event.detail, 158, evidenceY - 14, 375, body, 7.5, 11) - 13
  })

  const review = addPage('AI findings and human checklist', 'Explainable review')
  ...
  record.issues.slice(0, 5).forEach((issue) => {
    ...
    review.drawText(issue.title.slice(0, 58), { x: 130, y: issueY - 22, font: bold, size: 8.5, color: colors.ink })
  ...
  record.citations.forEach((citation, index) => {
    review.drawText(`${index + 1}. ${citation.label} - ${citation.section}`, { x: 49, y: issueY, font: bold, size: 7.5, color: colors.ink })
